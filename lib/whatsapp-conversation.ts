...
import { downloadWhatsAppMedia, sendWhatsAppActionMenu, sendWhatsAppAddressOptions, sendWhatsAppSiteOptions, sendWhatsAppText } from "@/lib/whatsapp-cloud";
...
export async function processWhatsAppConversationMessage(
  ...
  try {
    ...
    await ref.set({
      ...
      processingMessageId: "",
      phase: readyToBuild ? "address" : "intake",
      addressSuggestions: [],
      updatedAt: new Date().toISOString(),
    ...
    if (readyToBuild) {
      await sendWhatsAppText(
        sender,
        "What is the business street address? Send the street number, street name, and suburb or town. You can also reply “skip” if the business has no public address.",
      );
    }
  } catch (error) {
    await ref.set({ processingMessageId: "", updatedAt: new Date().toISOString() }, { merge: true });
...
export async function readWhatsAppHandoff(token: string): Promise<{
  ...
  return {
    ...
    addressResolved: ["complete", "payment", "site_ready"].includes(stringValue(data.phase)),
